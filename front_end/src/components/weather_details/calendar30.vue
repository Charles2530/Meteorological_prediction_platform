<template>
  <div class="common-layout" style="max-height: 85vh; overflow-y: auto">
      ...
      <el-main class="bg-cyan-100">
        <el-calendar class="rounded-lg italic">
          <template #date-cell="{ data }">
            ...
            <el-row>
              <span
                class="p-1 text-xl shadow-sm rounded-full"
                style="
                  font-style: normal;
        ...
        <el-card shadow="always">
              ...
              <el-col :span="12">
                <div class="grid-content">
                  <span style="font-size: 36px"
                    >{{ weatherTemp[0].min_temp }}~{{
                      weatherTemp[0].max_temp
                    }}℃</span
                  >
          ...
          <div class="grid grid-cols-4 gap-4 h-full mt-6">
            <template v-for="(item, index) in displayInfo" :key="index">
              <div
                v-if="item.data"
                class="flex flex-col text-center items-center gap-2"
              >
                <img
                  :src="item.icon"
                  :alt="item.name"
...
const get_data = async () => {
  get<WeatherData>(
    "/api/weather/30days",
    { city: city.value.name + city.value.adm2 },
    { month: month.value }
  ).then((res) => {
